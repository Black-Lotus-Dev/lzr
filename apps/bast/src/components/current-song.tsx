...
type SongEvent = "start" | "end" | "song" | "progress" | 1 | 0;

function CurrentSongOverlay() {
	//states
	const [song, setSong] = useState<BaseSong>();
	const [eventState, setEventState] = useState<SongEvent | undefined>();
	...
	function startPlaying(newSong) {
		isPlayingRef.current = true;
		setEventState("start");
		setSong(newSong);
	}

	function endPlaying(newSong) {
	...
	function runAnim() {
		if (!eventState || eventState === "progress") return;
